"""``ModelTrainer`` — plain-Python extraction of legacy ``ModelMetricsDataClay``.

The legacy class subclassed ``DataClayObject`` and wrapped its training
methods with ``@activemethod``. Neither contributed to the training
logic itself — they were artefacts of how the ICOS distribution shipped
models. Phase 1 strips both. Method signatures and return shapes are
preserved verbatim so the three ``oasis/models/*_compiler.py`` callers
keep working without changes.

Phase 2 will swap the data source from ``oasis/processing/`` to the
``TelemetrySource`` adapter without touching this file.
"""
...
class ModelTrainer:
    ...
    def make_persistent(self, alias: str | None = None) -> None:
        # Legacy compilers call this under the deprecated ``args.dataclay``
        # branch. Now a no-op — the trained model is what gets persisted,
        # via BentoML save_model(), inside each train_* method below.
        return None
